...
/// the [TemboForm] widget as a common ancestor of all of those. Call methods on
/// [TemboFormState] to save, reset, or validate each [FormField] that is a
/// descendant of this [TemboForm]. To obtain the [TemboFormState], you may use [TemboForm.of]
/// with a context whose ancestor is the [TemboForm], or pass a [GlobalKey] to the
/// [TemboForm] constructor and call [GlobalKey.currentState].
///
/// {@tool dartpad}
/// This example shows a [TemboForm] with one [TextFormField] to enter an email
/// address and an [ElevatedButton] to submit the form. A [GlobalKey] is used here
/// to identify the [TemboForm] and validate input.
///
...
class TemboForm extends StatefulWidget {
  ...
  /// Returns the [TemboFormState] of the closest [TemboForm] widget which encloses the
  /// given context, or null if none is found.
  ///
  /// Typical usage is as follows:
  ///
  /// ```dart
  /// TemboFormState? form = TemboForm.maybeOf(context);
  /// form?.save();
  /// ```
  ///
  /// Calling this method will create a dependency on the closest [TemboForm] in the
  /// [context], if there is one.
  ///
  /// See also:
  ///
  /// * [TemboForm.of], which is similar to this method, but asserts if no [TemboForm]
  ///   ancestor is found.
  static TemboFormState? maybeOf(BuildContext context) {
  ...
  ///
  /// ```dart
  /// TemboFormState form = TemboForm.of(context);
  /// form.save();
  /// ```
  ///
  /// If no [TemboForm] ancestor is found, this will assert in debug mode, and throw
  ...
  /// See also:
  ///
  /// * [TemboForm.maybeOf], which is similar to this method, but returns null if no
  ///   [TemboForm] ancestor is found.
  static TemboFormState of(BuildContext context) {
    final TemboFormState? formState = maybeOf(context);
    assert(() {
      if (formState == null) {
        throw FlutterError(
          'TemboForm.of() was called with a context that does not contain a TemboForm widget.\n'
          'No TemboForm widget ancestor could be found starting from the context that '
          'was passed to TemboForm.of(). This can happen because you are using a widget '
          'that looks for a TemboForm ancestor, but no such ancestor exists.\n'
  ...
  @override
  TemboFormState createState() => TemboFormState();
}

/// State associated with a [TemboForm] widget.
///
/// A [TemboFormState] object can be used to [save], [reset], and [validate] every
/// [FormField] that is a descendant of the associated [TemboForm].
///
/// Typically obtained via [TemboForm.of].
class TemboFormState extends State<TemboForm> {
  int _generation = 0;
  ...
  /// Saves every [FormField] that is a descendant of this [TemboForm].
  void save() {
  ...
  /// Resets every [FormField] that is a descendant of this [TemboForm] back to its
  /// [FormField.initialValue].
  ///
  /// The [TemboForm.onChanged] callback will be called.
  ///
  /// If the form's [TemboForm.autovalidateMode] property is [AutovalidateMode.always],
  ...
  /// Validates every [FormField] that is a descendant of this [TemboForm], and
...
class _FormScope extends InheritedWidget {
  const _FormScope({
    ...
    required TemboFormState formState,
    required int generation,
  })  : _formState = formState,
        _generation = generation;

  final TemboFormState _formState;

  /// Incremented every time a form field has changed. This lets us know when
  /// to rebuild the form.
  final int _generation;

  /// The [TemboForm] associated with this widget.
  TemboForm get form => _formState.widget;
...
///
/// When used inside a [TemboForm], you can use methods on [TemboFormState] to query or
/// manipulate the form data as a whole. For example, calling [TemboFormState.save]
...
///
/// A [TemboForm] ancestor is not required. The [TemboForm] allows one to
/// save, reset, or validate multiple fields at once. To use without a [TemboForm],
/// pass a [GlobalKey] to the constructor and use [GlobalKey.currentState] to
/// save or reset the form field.
///
/// See also:
///
///  * [TemboForm], which is the widget that aggregates the form fields.
...
class FormFieldState<T> extends State<FormField<T>> with RestorationMixin {
  ...
  // todo: find a better way to modify flutter error handling for textfields
  ...
  void reset() {
    ...
    TemboForm.maybeOf(context)?._fieldDidChange();
  }
  ...
  ///
  /// Triggers the [TemboForm.onChanged] callback and, if [TemboForm.autovalidateMode] is
  ...
  @override
  void deactivate() {
    TemboForm.maybeOf(context)?._unregister(this);
    super.deactivate();
  }
  ...
  Widget build(BuildContext context) {
    ...
    TemboForm.maybeOf(context)?._register(this);
    return widget.builder(this);
  }
}

/// Used to configure the auto validation of [FormField] and [TemboForm] widgets.
enum AutovalidateMode {
  /// No auto validation will occur.
  disabled,

  /// Used to auto-validate [TemboForm] and [FormField] even without user interaction.
  always,

  /// Used to auto-validate [TemboForm] and [FormField] only after each user
  /// interaction.
  onUserInteraction,
}
